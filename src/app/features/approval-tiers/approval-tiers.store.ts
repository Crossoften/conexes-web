...
import { ApprovalTier, ApprovalTierStatus } from './approval-tiers.model';
import { APPROVAL_TIERS_MOCK } from './approval-tiers.mock';

interface State {
  items: ApprovalTier[];
  loading: boolean;
  filters: { search: string; status: ApprovalTierStatus | '' };
...
export class ApprovalTiersStore {
  ...
  toggleAllPage(items: ApprovalTier[]) {
    this.state.update(s => {
      const newSet = new Set(s.selectedIds);
