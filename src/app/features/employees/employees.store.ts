...
import { Employee, EmployeeStatus, EmployeeType } from './employees.model';
import { EMPLOYEES_MOCK } from './employees.mock';

interface State {
  items: Employee[];
  loading: boolean;
  filters: { search: string; status: EmployeeStatus | ''; type: EmployeeType | '' };
...
export class EmployeesStore {
  ...
  readonly filteredItems = computed(() => {
    ...
    if (f.type) { result = result.filter(item => item.type === f.type); } // Filtro genérico pelo texto
  ...
  setStatus(status: EmployeeStatus | '') { this.state.update(s => ({ ...s, filters: { ...s.filters, status }, pagination: { ...s.pagination, page: 1 } })); }
  setType(type: EmployeeType | '') { this.state.update(s => ({ ...s, filters: { ...s.filters, type }, pagination: { ...s.pagination, page: 1 } })); }

  setSort(column: keyof Employee) {
    this.state.update(s => {
